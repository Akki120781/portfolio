...
<CodeBlock
  ...
  {
    code:
`import { InlineCode } from "@akki/core";
...
Social links are rendered on the `/about` page and in the footer based on the array configured in the `social` object. You can set custom icons to each, but don't forget to import them in `src/once-ui/icons.ts`. Read the akki documentation](https://once-ui.com/docs/icons) for more information.
...
`const social = [
    {
      name: "GitHub",
      icon: "github",
      link: "https://github.com/akki/nextjs-starter",
    },
    {
      name: "Email",
      icon: "email",
